...
VALID_CLI_OPTION=("--all" "--lint" "--unit-test" "--help" "-h")
...
#  fi
  ...
  if [[ ${STEPS_ACTIVE_MAP[black]} == "true" ]]; then
    ...
    eval "black ${BLACK_ACTION} ${STEP_EXTRA_ARGUMENTS[black]} ."
  fi
  ...
  if [[ ${STEPS_ACTIVE_MAP[mypy]} == "true" ]]; then
    ...
    eval "mypy --check-untyped-defs ${STEP_EXTRA_ARGUMENTS[mypy]} ."
  fi

  if [[ ${STEPS_ACTIVE_MAP[pytest]} == "true" ]]; then
    echo "Running pytest..."
    # Test discovery is controlled by settings in pyproject.toml
    eval "CL_UTILITIES_CONFIG=\"testing\" pytest --cov=components -vv ${STEP_EXTRA_ARGUMENTS[pytest]}"
  fi
  ...
  if [[ ${STEPS_ACTIVE_MAP[bandit]} == "true" ]]; then
    ...
    # Explicitly specifying settings file because using a different path will cause the file to be ignorec
    eval "bandit --ini .bandit -rq ${STEP_EXTRA_ARGUMENTS[bandit]} ."
  fi
}
